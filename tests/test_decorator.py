...
class TestInjectCaching:
    def test_caches_across_multiple_params(self) -> None:
        call_count = 0
    ...
    def test_no_cache_calls_each_time(self) -> None:
        call_count = 0
        ...
        @inject
        def handler(
            a: int = Needs(counted, use_cache=False),
            b: int = Needs(counted, use_cache=False),
        ) -> tuple[int, int]:
            return a, b
